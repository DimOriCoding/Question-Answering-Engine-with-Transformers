<!-- GitHub-stable mirror of QA_engine_with_difflib_for_SimpleQuestions.ipynb. Notebook cell sources are preserved; outputs are intentionally omitted. -->
...
**Εισαγωγή αρχείου με τις ερωτήσεις που θα χρησιμοποιηθούν για τα Question Answering Engines, που συμπεριλαμβάνει τις απαντήσεις τους.**
...
**Έλεγχος για τυχόν null values σε κάποια γραμμή για τα αντίστοιχα datasets και στο αρχείο με τις ερωτήσεις.**
...
**Δημιουργία συνάρτησης get_entity_id που δέχεται σαν όρισμα κάποιο entity label, η οποία περιλαμβάνει τη δημιουργία ενός SPARQL query, το οποίο δοθέντος ενός entity label, βρίσκει και εξάγει το αντίστοιχο entity id του.**
...
**Creation of a dictionary named Endict, which has as keys the entity labels of the training validation and test dataset and as values the corresponding entity ids of the training, validation and test dataset.**
...
def find_closest_match(entity_label, Endict, entities):
      #Find the best match from the list of entity labels
      best_match = difflib.get_close_matches(entity_label, entities, n=1)
      if best_match != []:
        print("Spacy library is used!")
...
```python
def entity_prediction(ids, mask, Endict, entities, tokenizer, max_question_length, activation, entity_model_path, dropout_prob, SEED):
    ...
    if entity in Endict:
        ...
        if entityid != []:
            return entityid[0]
    # Fallback to the most similar from the entities list of the dictionary
    return find_closest_match(entity, Endict, entities)
```
...
```python
def answer(question, Endict, entities, rel_vocab, rel_vocab_len, tokenizer, max_question_length, activation_relation, activation_entity, relation_model_path, entity_model_path, dropout_prob, SEED, real_answer, real_entity_id, real_relation_id):
    ...
    entity = entity_prediction(ids, mask, Endict, entities, tokenizer, max_question_length, activation_entity, entity_model_path, dropout_prob, SEED)
    print(entity)
    if entity == None:
        ...
        if answers == []:
          ...
          for iter, answer in enumerate(model_answers):
              print('answer: ', answer)
          ans = difflib.get_close_matches(real_answer, model_answers, n=5)
          if entity != real_entity_id and relation != real_relation_id:
            ...
            if ans != []:
             ...
             question_answered = 1
             print("The question is answered with either only correct answers or only incorrect answers or both of them, but with not the expected answer!")
          else:
            if ans != []:
             question_correct_answered = 1
...
while(1):
  ...
  for i in range(len(testing_questions)):
    if testing_questions[i] == question:
      ...
      break
  question_correct_answered, question_answered = answer(question, Endict, entities, rel_vocab, len(rel_vocab), tokenizer, max(q_lengths), "ELU", "ELU", activation_relation_best_parameters_path, activation_entity_best_parameters_path, dropout_prob, SEED, real_question_answer_label, real_question_entity_id, real_question_relation_id)
...
print("The percentage of the test questions that are answered but not with the expected answer is: {:.2f}%".format((count_test_questions_answered/count_test_questions_total)*100))
```
...
while(1):
  ...
  for i in range(len(testing_questions)):
    if testing_questions[i] == question:
      ...
      real_question_relation_id = testing_questions_relation_ids[i]
      break
  question_correct_answered, question_answered = answer(question, Endict, entities, rel_vocab, len(rel_vocab), tokenizer, max(q_lengths), "ELU", "Softplus", activation_relation_best_parameters_path, activation_entity_best_parameters_path, dropout_prob, SEED, real_question_answer_label, real_question_entity_id, real_question_relation_id)
  if question_correct_answered == 1:
    count_test_questions_correct_answered += 1
  if question_answered == 1:
    count_test_questions_answered += 1
...
#Find the real answer, relation id and entity id of the input question from the corresponding .txt file.
  for i in range(len(testing_questions)):
    if testing_questions[i] == question:
      ...
      real_question_relation_id = testing_questions_relation_ids[i]
      break
  question_correct_answered, question_answered = answer(question, Endict, entities, rel_answerable_vocab, len(rel_answerable_vocab), tokenizer, max(q_lengths), "ELU", "Softplus", activation_relation_best_parameters_path, activation_entity_best_parameters_path, dropout_prob, SEED, real_question_answer_label, real_question_entity_id, real_question_relation_id)
  if question_correct_answered == 1:
    count_test_questions_correct_answered += 1
  if question_answered == 1:
    count_test_questions_answered += 1
...
while(1):
  ...
  for i in range(len(testing_questions)):
    if testing_questions[i] == question:
      ...
      real_question_relation_id = testing_questions_relation_ids[i]
      break
  question_correct_answered, question_answered = answer(question, Endict, entities, rel_answerable_vocab, len(rel_answerable_vocab), tokenizer, max(q_lengths), "ELU", "ELU", activation_relation_best_parameters_path, activation_entity_best_parameters_path, dropout_prob, SEED, real_question_answer_label, real_question_entity_id, real_question_relation_id)
  if question_correct_answered == 1:
    count_test_questions_correct_answered += 1
  if question_answered == 1:
    count_test_questions_answered += 1
