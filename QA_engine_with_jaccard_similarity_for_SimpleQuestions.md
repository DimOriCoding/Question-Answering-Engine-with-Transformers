<!-- GitHub-stable mirror of QA_engine_with_jaccard_similarity_for_SimpleQuestions.ipynb. Notebook cell sources are preserved; outputs are intentionally omitted. -->
...
def find_closest_match(entity_label, Endict, entities):
      ...
      for ent_label in entities:
          similarity_scores[ent_label] = jaccard_similarity(entity_label.lower().split(),ent_label.lower().split())
      ...
      if EntitiesList != []:
        ...
        print("Jaccard similarity metric is used!")
        return entityid
      else:
        return None
```
    ...
    if entity == None:
        ...
        if answers == []:
          ...
          for iter, answer in enumerate(model_answers):
              answer_similarity_scores[answer] = jaccard_similarity(real_answer.split(),answer.split())
...
print("The percentage of the test questions that are answered either with either only correct or only incorrect answers or both of them,\nregardless if the expected answer is one of the possible correct answer(s) or not: {:.2f}%".format((count_test_questions_answered/count_test_questions_total)*100))
```
