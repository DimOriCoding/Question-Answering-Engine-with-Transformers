...
The notebooks that are included in this file display the development of the Question Answering Engine modules. Firstly the entity label and also the relation id prediction is made. Secondly an SPARQL query is constructed to obtain the ID of the entity label. If the entity id is not found, i.e the query is unsuccesful, the QA engine system searches for the closest match either through Sentence Transformers similarity model or a similarity metric such as cosine and jaccard. Finally the relation id and the entity id that are occurred from the corresponding predictions are combined through another SPARQL query in order to find the final answer if this exists. Essentially in this file you can see and explore all the examples of the Question Answering Engines (Sentence Transformers library, cosine similarity metric, jaccard similarity metric). Also must be mentioned that a file named [**Examples_with_cosine_and_jaccard_similarity_metric**] (https://github.com/DimOriCoding/Question-Answering-Engine-with-Transformers/blob/main/Question%20Answering%20Engines/Examples_with_cosine_and_jaccard_similarity_metric.ipynb) is created in order to see which of cosine or jaccard similarity metric performs better on sentences/text similarity, with cosine similarity metric to perform better. Browser-friendly Python source code is available in [**QA_Engine_with_difflib_Results.py**](https://github.com/DimOriCoding/Question-Answering-Engine-with-Transformers/tree/main/QA_Engine_with_difflib_Results.py), [**QA_Engine_Sentence_Transformers_Cosine_Similarity_Results.py**](https://github.com/DimOriCoding/Question-Answering-Engine-with-Transformers/tree/main/QA_Engine_Sentence_Transformers_Cosine_Similarity_Results.py) and [**QA_Engine_Jaccard_Similarity_Results.py**](https://github.com/DimOriCoding/Question-Answering-Engine-with-Transformers/tree/main/QA_Engine_Jaccard_Similarity_Results.py). Additionally three .txt files are created and contain all the results of Question Answering Engines that are created (difflib, Sentence Transformers + Cosine similarity metric, Jaccard similarity metric) for both SimpleQuestions datasets (including either all questions (answerable, not answerable over Wikidata) or only answerable over Wikidata questions)
and all the relation prediction methods (relation vocabulary) and entity span prediction methods (entity span encoded with 0 and 1 ignoring or not questions 
with no equality or similarity with the enitiy label that SPARQL queries give for each question, combination of Sentence Transformer and cosine similarity metric 
and jaccard similarity metric).
